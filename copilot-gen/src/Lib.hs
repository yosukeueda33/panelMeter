...
alwaysBeen' :: Stream Word16 -> Stream Bool -> Stream Bool
alwaysBeen' len s = cnt >= (len - 1) -- && s
  where
    cnt = [0] ++ mux s (cnt + 1) 0


once' :: Stream Word16 -> Stream Bool -> Stream Bool
once' len s = cnt > 0
  where
    cnt = [0] ++ mux s len (lim 0 $ cnt - 1)
    lim l x = mux (l < x) x l

timer :: Stream Word16 -> Stream Bool -> Stream Bool 
...
debounce :: Stream Word16 -> Stream Bool -> Stream Bool 
